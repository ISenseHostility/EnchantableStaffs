...
public class LightningCircle extends Enchantment implements IStaffEnchantment {
    ...
    public InteractionResultHolder<ItemStack> onUse(ItemStack stack, Level level, Player player) {
        ...
        for (int i = 0; i < 8; i++) {
            bolts[i] = createLightningBolt(level);
        }

        double x = player.getX();
        double z = player.getZ();

        bolts[0].setPos(x + 10, getHighestBlock(level, x + 10, z).above().getY(), z);
        bolts[1].setPos(x, getHighestBlock(level, x, z + 10).above().getY(), z + 10);
        bolts[2].setPos(x - 10, getHighestBlock(level, x - 10, z).above().getY(), z);
        bolts[3].setPos(x, getHighestBlock(level, x, z - 10).above().getY(), z - 10);
        bolts[4].setPos(x + 5, getHighestBlock(level, x + 5, z + 5).above().getY(), z + 5);
        bolts[5].setPos(x - 5, getHighestBlock(level, x - 5, z - 5).above().getY(), z - 5);
        bolts[6].setPos(x + 5, getHighestBlock(level, x + 5, z - 5).above().getY(), z - 5);
        bolts[7].setPos(x - 5, getHighestBlock(level, x - 5, z + 5).above().getY(), z + 5);

        if (!level.isClientSide) {
            for (LightningBolt bolt : bolts) {
                level.addFreshEntity(bolt);
                spawnParticleCloud(ParticleTypes.FIREWORK, bolt.getX() + 0.5D, bolt.getY() + 1.0D, bolt.getZ() + 0.5D, level);
            }
        }
