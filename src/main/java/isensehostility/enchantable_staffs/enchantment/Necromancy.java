...
public class Necromancy extends Enchantment implements IStaffEnchantment {
    ...
    public InteractionResultHolder<ItemStack> onUse(ItemStack stack, Level level, Player player) {
        ...
        NBTUtils.setFriendly(entity, true);
        entity.setPos(Vec3.atCenterOf(pos));
        level.addFreshEntity(entity);

        ModUtils.spawnParticleCloud(ParticleTypes.SOUL, level, pos.getX(), pos.getY() + 1.0D, pos.getZ());
        ModUtils.spawnParticleCloud(ParticleTypes.SOUL, level, player.getX(), player.getEyeY(), player.getZ());
